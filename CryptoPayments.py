...
class CryptoPayments():
    ...
    def getTransactionInfo(self,params={}):
        """Gets transaction info based on given transaction id
        """
    ...
    def claimTag(self,params={}):
        """Claims tag with transaction id and date, so multiple can't use same tID
        """
        params.update({'cmd':'claim_pbn_tag',
                        'key':self.publicKey,
                       'version': self.version,
                       'format': self.format})
        return self.Request('post',**params)

    def getTagList(self,params={}):
        """Grabs tag list
        """
        params.update({'cmd':'get_pbn_list',
                        'key':self.publicKey,
                       'version': self.version,
                       'format': self.format})
        return self.Request('post',**params)
